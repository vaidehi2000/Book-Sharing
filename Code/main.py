    ...
    if valid_boolean:
        ...
        if choice == "1":
            ...
            if len(filename) != 0:
                ...
                peer_host, peer_port, download_it = show_result(
                    file_data, filename)
                if download_it:
                    peer.download_file([DOWNLOAD, filename],
                                       peer_host, peer_port)
                else:
            ...
            valid_boolean, valid_host, valid_port = validate_ip_port(
                PEER_PORT, PEER_HOST)
            if valid_boolean:
                ...
                try:
                    shared_files = [f for f in listdir(
                        PATH) if path.isfile(path.join(PATH, f))]
                    if len(shared_files) != 0:
                        ...
                        if REGISTERED_SUCCESSFULLY:
                            DATA_INSERTED = peer.data_object.append_data(
                                data_object)
                            if DATA_INSERTED:
                                ...
                                print(
                                    "There was an error while inserting your data.")
                    else:
